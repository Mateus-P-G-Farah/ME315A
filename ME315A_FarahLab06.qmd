...
```{r}
dbGetQuery(conn, "SELECT COUNT(*) FROM customers AS total_de_clientes;")
```
6.  Utilizando apenas SQLite, identifique o número de países diferentes em que moram os clientes encontrados acima.

```{sql connection=conn}
SELECT COUNT(DISTINCT COUNTRY) AS DISTINCT_COUNTRYS FROM CUSTOMERS
```
...
```{sql connection=conn}
SELECT COUNTRY, COUNT(*) AS TOTAL_DE_CLIENTES FROM CUSTOMERS
GROUP BY COUNTRY
