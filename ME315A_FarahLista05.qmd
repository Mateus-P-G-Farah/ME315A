...
```{sql connection=con}
SELECT AVG(ACRES) AS MEDIA_ACRES, MIN(ACRES) AS MINIMO_ACRES, MAX(ACRES) AS MAXIMO_ACRES FROM PRESERVE
WHERE STATE = "AZ"
```
...
```{sql connection=con}
SELECT COUNT(DISTINCT FEE) AS TAXAS_DE_ENTRADA_DISTINTAS FROM PRESERVE
WHERE FEE != 0
```
...
```{sql connection=con}
SELECT COUNT(PNAME) AS TOTAL, COUNT(DISTINCT PNAME) AS TOTAL_DISTINTOS FROM PRESERVE
```
...
```{sql connection=con}
SELECT SUM(SALARY) AS SOMA_SALARIO, AVG(SALARY) AS MEDIA_SALARIO, MAX(SALARY) AS MAXIMO_SALARIO, MIN(SALARY) AS MINIMO_SALARIO FROM EMPLOYEE
```
